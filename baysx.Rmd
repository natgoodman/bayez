...
2. Each line of `posterior` creates a function: e.g., the line `P_obsGIVENpop=function(d.pop) d_d2t(n,d.pop,d.obs);` creates a function of `d.pop` that calls the function `d_d2t` on variables `n`, `d.pop`, and `d.obs`. A further detail is that variables `n` and `d.obs` are *bound* to the values passed into `posterior` as arguments, while `d.pop` is *free* and gets its value later when the new function `P_obsGIVENpop` is called.
